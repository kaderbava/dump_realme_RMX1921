...
function fingerprintcache(){
    ...
    qseelog_enable=`getprop persist.enable.qseelog`

    if [ ${state} != "0" ] || [ x"${qseelog_enable}" = x"true" ]
    then
...
function PreprocessLog(){
    ...
    if [ "${panicenable}" = "${argtrue}" ] || [ x"${camerapanic}" = x"true" ]; then
        ...
        ROOT_SDCARD_uefi_LOG_PATH=${ROOT_SDCARD_LOG_PATH}/uefi
        mkdir -p  ${ROOT_SDCARD_uefi_LOG_PATH}
    ...
    if [ "${panicenable}" = "${argtrue}" ] || [ x"${camerapanic}" = x"true" ]; then
        ...
        #Qi.Zhang@TECH.BSP.Stability 2019/09/20, Add for uefi log
        setprop sys.oppo.logkit.uefilog ${ROOT_SDCARD_uefi_LOG_PATH}
...
if [ ${FreeSize} -ge ${GSIZE} ]; then
    ...
    #Qi.Zhang@TECH.BSP.Stability 2019/09/20, Add for uefi log
    ROOT_SDCARD_uefi_LOG_PATH=`getprop sys.oppo.logkit.uefilog`
...
function LogcatUefi(){
    ...
    if [ "${panicenable}" = "${argtrue}" ] || [ x"${camerapanic}" = x"true" ];then
        ...
        mv  ${CACHE_PATH}/uefi/cur_uefi_log  ${ROOT_SDCARD_uefi_LOG_PATH}/cur_uefi_log.txt
    fi
}
...
function tranfer2TfCard(){
    ...
    rm -rf `find data/oppo/log/DCS -type f`
    ...
    mv /sdcard/oppo_log/camera_monkey_log ${newpath}/
    ...
    curFile=/sdcard/oppo_log/screen_record
    if [ -d "$curFile" ]; then
         mv $curFile ${newpath}/
    fi
...
function tranfer2SDCard(){
    ...
    binder_info_dir="/sdcard/oppo_log/binder_info"
    if [ -d ${binder_info_dir} ]; then
        echo "copy binder_info..."
        mv /sdcard/oppo_log/binder_info ${newpath}/
    fi
...
function resetlogpath(){
    ...
    setprop sys.oppo.logkit.uefilog ""
...
function save_latest_log() {
    ...
    if [ -d ${LATEST_APPS_LOG_PATH} ]; then
        LOGTIME=`date +%F-%H-%M-%S`
        CAMERA_LOG_DIR=/sdcard/oppo_log/camera_monkey_log/log_${LOGTIME}
...
function binderinfocapture() {
    if [ ! -d /sdcard/oppo_log/binder_info/ ];then
    mkdir -p /sdcard/oppo_log/binder_info/
    fi

    LOGTIME=`date +%F-%H-%M-%S`
    BINDER_DIR=sdcard/oppo_log/binder_info/binder_${LOGTIME}
    echo ${BINDER_DIR}
    mkdir -p ${BINDER_DIR}
